'''
사실 이건 내 코드가 아니다.
다른 사람의 풀이를 가져왔다. (어떤 방법으로 접근해 풀어야 할 지 감이 안 잡히더라.)

그래서 결과적으로 이것도 BFS 로 푸는 문제였다.
처음 지뢰가 없는 곳을 클릭 했을 때, 연쇄적으로 다음 지뢰가 없는 곳을 탐색하여 풀어나아 가는 문제였다.

진짜 DFS 하고 BFS 로 푸는 문제들에 대해 공부를 많이 해야 어떻게 접근할 지 조금 보일 것 같다.
아직 공부해야 할 길이 멀다는 걸 알게 된 문제였던 것 같다.
'''
